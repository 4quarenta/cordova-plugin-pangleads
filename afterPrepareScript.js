...
module.exports = function (ctx) {
  ...
  const targetProguardFile = path.join(pluginDir, 'proguard.txt');
  const projectProguardFile = path.join(projectRoot, 'proguard.txt');
  ...
  try {
    if (fs.existsSync(projectProguardFile)) {
      ...
      console.log('No optional proguard.txt found in projectRoot: ' + projectRoot);
    }
  } catch(err) {
    console.error(err);
  }
